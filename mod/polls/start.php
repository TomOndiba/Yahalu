...
function polls_init() {
	...
	// Set up menu for logged in users
		if (isloggedin()) {    		
			add_menu(elgg_echo('polls'), $CONFIG->wwwroot . "polls/list/" . get_loggedin_user()->username);
			
	// And for logged out users
		} else {
			add_menu(elgg_echo('poll'), $CONFIG->wwwroot . "polls/all");
		}
	...
	// Register a URL handler for poll posts
		...
		register_entity_url_handler('polls_url_bfrens','object','poll_bfrens');
						
	// Register entity type
		register_entity_type('object','poll');
		 	elgg_register_entity_type('object','pollbfrens');
	...
	//add submenu options
		if (get_context() == "polls") {
			if ((page_owner() == $_SESSION['guid'] || !page_owner()) && isloggedin()) {
				add_submenu_item(elgg_echo('polls:your'),$CONFIG->wwwroot."polls/list/" . $page_owner->username);
				add_submenu_item(elgg_echo('polls:friends'),$CONFIG->wwwroot."polls/friends/" . $page_owner->username);
				add_submenu_item(elgg_echo('polls:everyone'),$CONFIG->wwwroot."polls/all");
				add_submenu_item(elgg_echo('polls:addpost'),$CONFIG->wwwroot."polls/add/" . $page_owner->username);
			} else if (page_owner()) {
				if ($page_owner instanceof ElggUser) { // Sorry groups, this isn't for you.
					add_submenu_item(sprintf(elgg_echo('polls:user'),$page_owner->name),$CONFIG->wwwroot."polls/" . $page_owner->username);
					add_submenu_item(sprintf(elgg_echo('polls:user:friends'),$page_owner->name),$CONFIG->wwwroot."polls/" . $page_owner->username . "/friends/");
				} else if ($page_owner instanceof ElggGroup) {
				//if (polls_can_add_to_group($page_owner)) 
                                        {
						add_submenu_item(elgg_echo('polls:addpost'),$CONFIG->wwwroot."polls/add/" . $page_owner->username);
					}
					add_submenu_item(elgg_echo('polls:group_polls'), $CONFIG->wwwroot . "polls/list/" . $page_owner->username);
				}
				add_submenu_item(elgg_echo('polls:everyone'),$CONFIG->wwwroot."polls/all");
			} else {
				add_submenu_item(elgg_echo('polls:everyone'),$CONFIG->wwwroot."polls/all");
	...
	if (isset($page[0])) {
		switch($page[0]) {
			...
			case "all":			@include(dirname(__FILE__) . "/pages/everyone.php");
								break;
			case "add_lightbox":			if (isset($page[1])) {
									set_input('username',$page[1]);
								}
								@include(dirname(__FILE__) . "/pages/add_lightbox.php");
								break;
...
function polls_url($pollpost) {
	...
	return $CONFIG->url . "polls/read/" . $pollpost->getOwnerEntity()->username . "/" . $pollpost->getGUID() . "/" . $title;
	
}
function polls_url_bfrens($pollpost) {
	
	global $CONFIG;
	$title = $pollpost->title;
	$title = friendly_title($title);
	return $CONFIG->url . "pollsbfrens/read/" . $pollpost->getOwnerEntity()->username . "/" . $pollpost->getGUID() . "/" . $title;
...
register_action("polls/add_lightbox",false,$CONFIG->pluginspath . "polls/actions/add.php");
